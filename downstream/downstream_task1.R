...
## Set up data paths
raw_dir <- "/challenge-data/raw/"
ds_dir <-  "/challenge-data/downsampled/scrna/"
output_dir <- "/output"
...
# Read team input
team <- commandArgs(trailingOnly = TRUE)[1]
...
if (!file.exists(refs_path)) {
  refs <- mclapply(datasets, function(dataset) {
    ...
    matrices_to_seurat(file.path(ds_dir, f),
                       ...
                       data_type = "reference",
                       condition = "control")
  }, mc.cores = ncores) %>% set_names(datasets)
...
for (dataset in datasets) {
  ...
  # control_querys_path <- file.path(output_dir, "control_querys.rds")
  # if (!file.exists(control_querys_path)) {
  #   control_querys <- pbmclapply(filenames, function(f) {

  #     data_path <- file.path(ds_dir, f)     

  #     if (!is_by_cells) { # just take one of replicates if downsampled by cells
  #       new_f_path <- paste0(data_path, "_n", 1, "_imputed.csv")
  #     } else {
  #     new_f_path <- aggregate_output_by_mean(data_path_list = paste0(data_path, "_n", 1:3, "_imputed.csv"),
  #                                           new_filename = str_glue("{team}_{f}_imputed.csv"))
  #     }
     
  #     matrices_to_seurat(new_f_path,
  #                       nickname = str_glue("{f}"),
  #                       cells = valid_cells[[dataset]],
  #                       dataset = dataset,
  #                       data_type = "query",
  ...
  #   }, mc.cores = ncores) %>% set_names(filenames)
  #   saveRDS(control_querys, control_querys_path)
  # }

  querys <-  pbmclapply(filenames, function(f) {
    print(f)
    data_path <- file.path(output_dir, team, "output", f)
    is_by_cells <-  str_split(basename(f), "_")[[1]][2] %in% ds_cells_props

    new_filename <- str_glue("{team}_{f}_imputed.csv")
    if (is_by_cells) { # just take one of replicates if downsampled by cells
      imp <- fread(paste0(data_path, "_n", 1, "_imputed.csv")) %>% tibble::column_to_rownames("V1")
      fwrite(imp, new_filename, row.names = TRUE)
    } else {
      new_data <- aggregate_output_by_mean(data_path_list = paste0(data_path, "_n", 1:3, "_imputed.csv"),
                                           new_filename = new_filename)
    }
    
    matrices_to_seurat(new_filename,
  ...
  out <- pbmclapply(seq_along(querys), function(i) {
    # allow_cell_mismatch <- str_split(names(querys)[i], "_")[[1]][2] %in% ds_cells_props
...
for (g in groups) {
  ...
  }, mc.cores = ncores) %>% set_names(gsub(str_glue("{team}_"), "", comparisons_to_make$control_nicknames))
