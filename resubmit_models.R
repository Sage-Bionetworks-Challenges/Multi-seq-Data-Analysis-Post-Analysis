...
task_sub_df <- task_sub_df %>%
  filter(!submitterid %in% names(is_dup_user),
         !team %in% c("scRNA-seq and scATAC-seq Data Analysis DREAM Challenge Organizers", "rchai")) %>%
...
if (nrow(task_sub_df) > 0) {
  ...
  for (n in seq_along(sub_ids)[1]) {
    ...
    if (!dry_run) {
      ...
      # wait 15 min to check if submission is scored
      # cp <- 0
      # while (cp <= 0) {
      #   Sys.sleep(15 * 60)
      ...
      # idk why submitterid cannot be overwrote, so use new column
      team_id <- task_sub_df$submitterid[match(sub_ids[n], task_sub_df$id)]
      annots <- list(
        re_submission_phase = as.character(phase),
        re_submitterid = as.character(team_id)
      )
